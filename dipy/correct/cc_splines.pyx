...
def cc_splines_gradient_epicor(double[:,:,:] g, double[:,:,:] f,
                               double[:,:,:] dg, double[:,:,:] df,
                               double pedir_factor,
                               int[:,:,:] mf, int[:,:,:] mg,
                               double[:,:,:] kernel, double[:,:,:] dkernel,
    ...
    cdef:
        ...
        int* kcenter = [ksize[0]//2, ksize[1]//2, ksize[2]//2]
        double[:,:,:] F = np.empty((ns,nr,nc), dtype=np.float64)
        double[:,:,:] G = np.empty((ns,nr,nc), dtype=np.float64)
        double[:, :, :, :] temp = np.zeros((ns, nr, nc, 5), dtype=np.float64)
        int nfactors = 3
        ...
        cnp.npy_intp sss, ss, rr, cc, prev_sss, prev_rr, prev_cc
        int s, r, c, i, j, k, i0, i1, j0, j1, k0, k1, sc_s, sc_r, sc_c, sides, sider, sidec, nwindows
        double Jf, Jg, Fbar, Gbar, alpha, beta, gamma, sp_contrib, dF, dG, A, B, C, energy
        double deltay = kspacing[1]
    kcoef[...] = 0
    with nogil:
        for s in range(ns):
            for r in range(nr):
                for c in range(nc):
                    Jf = 1.0 + pedir_factor * dfield[s,r,c]  # local Jacobian
                    Jg = 1.0 - pedir_factor * dfield[s,r,c]  # local Jacobian
                    F[s,r,c] = f[s,r,c] * Jf
                    G[s,r,c] = g[s,r,c] * Jg
                            ...
                            for k in range(k0, 1 + k1):
                                ...
                                for i in range(i0, 1 + i1):
                                    ...
                                    for j in range(j0, 1 + j1):
                                        ...
                                        Jf = 1 + pedir_factor * dfield[ss,rr,cc]  # local Jacobian (for F)
                                        Jg = 1 - pedir_factor * dfield[ss,rr,cc]  # local Jacobian (for G)

                                        dF = pedir_factor * (df[ss, rr, cc] * kernel[ss - sc_s, rr - sc_r, cc - sc_c] * Jf +
                                             Fbar * dkernel[ss - sc_s, rr - sc_r, cc - sc_c] / deltay)
                                        dG = pedir_factor * (-1.0*dg[ss, rr, cc] * kernel[ss - sc_s, rr - sc_r, cc - sc_c] * Jg +
                                             Gbar * dkernel[ss - sc_s, rr - sc_r, cc - sc_c] / deltay)

                                        sp_contrib = ((alpha * Gbar - beta * Fbar) * dF +
                                                     (alpha * Fbar - gamma * Gbar) * dG)
