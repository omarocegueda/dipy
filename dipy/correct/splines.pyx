...
cdef class Spline:
    cdef:
        ...
        double* splines[2] # Precomputed kernel and derivative
    ...
    cdef void _fit_to_data(self, double[:] data, double tau, double[:]out):
        cdef:
            int n = data.shape[0]
            int ncoef = self._knots_needed(n)
            double[:,:] At = np.zeros((ncoef, n), dtype=np.float64)
            double[:,:] AtA = np.empty((ncoef, ncoef), dtype=np.float64, order='C')
            double[:] Aty = np.empty(ncoef, dtype=np.float64, order='C')
        ...
        with nogil:
            for j in range(ncoef):
                self._grid_cells_affected(j, n, &first, &last, &spline_offset)
    ...
    cpdef fit_to_data(self, double[:] data, double[:] out=None, double tau = 0.0001):
        cdef:
            int ncoef = self._knots_needed(data.shape[0])
        if out is None:
            out = np.ndarray(ncoef, dtype=np.float64)
        self._fit_to_data(data, tau, out)
        return out
    ...
    def __dealloc__(self):
        PyMem_Free(self.splines[0])
        PyMem_Free(self.splines[1])
...
cdef class Spline3D:
    cdef:
        ...
        Spline sy
        Spline sz
        double *splines[8] # precomputed spline and partial derivatives
    ...
    def __cinit__(self, Spline sx, Spline sy, Spline sz):
        ...
        order = 0 # Derivative order (0 to 7)
        for dx in range(2):
            for dy in range(2):
                for dz in range(2):
                    ...
                    for x in range(sx.kernel_size):
                        for y in range(sy.kernel_size):
                            for z in range(sz.kernel_size):
                                self.splines[order][pos] = sx.splines[dy][x] *\
                                                           sy.splines[dy][y] *\
                                                           sz.splines[dz][z]
                                pos += 1
                    order += 1

    cdef void _eval_spline_field(self, double[:,:,:] coefs, int[:] der,
                                 double[:,:,:] out)nogil:
        cdef:
            ...
            int last[3]
            int offset[3]
            double *spline = self.splines[der[0] * 4 + der[1] * 2 + der[2]]
            double coef
    ...
    cpdef fit_axis(self, double[:,:,:] vol, int axis, double tau=0.0001):
        ...
        if axis == 0:
            ...
            coef = np.ndarray(nkx, dtype=np.float64)
            out = np.ndarray((nkx, ly, lz), dtype=np.float64)
            for y in range(ly):
                for z in range(lz):
                    self.sx.fit_to_data(vol[:,y,z], coef, tau)
                    with nogil:
                        for x in range(nkx):
                            out[x,y,z] = coef[x]
            return out
            ...
            coef = np.ndarray(nky, dtype=np.float64)
            out = np.ndarray((lx, nky, lz), dtype=np.float64)
            for x in range(lx):
                for z in range(lz):
                    self.sy.fit_to_data(vol[x,:,z], coef, tau)
                    with nogil:
                        for y in range(nky):
                            out[x,y,z] = coef[y]
            return out
            ...
            coef = np.ndarray(nkz, dtype=np.float64)
            out = np.ndarray((lx, ly, nkz), dtype=np.float64)
            for x in range(lx):
                for y in range(ly):
                    self.sz.fit_to_data(vol[x,y,:], coef, tau)
                    with nogil:
                        for z in range(nkz):
                            out[x,y,z] = coef[z]
            return out
...
class CubicSplineField:
    ...
    def get_volume(self, der_orders = (0,0,0)):
        ...
        self.spline3d._eval_spline_field(self.coef, der_orders, volume)
