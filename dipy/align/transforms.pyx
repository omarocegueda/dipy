...
def number_of_composite_parameters(int[:] ttypes, int dim):
    r""" Number of parameters of a composite transform

    The total number of parameters is the sum of the number of parameters
    of all the transform types that define the composite transform

    Parameters
    ----------
    ttypes : array, shape(m,)
        the types of the transform that define the composite transform
        ttypes[0] is the type of the transform that is evaluated first
    dim : int
        the domain dimension of the transformation (either 2 or 3)
    ...
    -------
    n : int
        the number of parameters of a composite transform
    """
    return _number_of_composite_parameters(ttypes, dim)


cdef int _number_of_composite_parameters(int[:] ttypes, int dim) nogil:
    cdef:
        int n = 0
        int m = ttypes.shape[0]
        int i
    for i in range(m):
        n += _number_of_parameters(ttypes[i], dim)
    return n
...
def get_composite_identity(int[:] ttypes, int dim, double[:] theta):
    r""" Gets the parameters corresponding to the identity composite transform

    The composite transform is given by a sequence of transforms of types given
    by the ttypes array and dimension dim. The identity parameters for
    each transform are returned in theta in the same order as ttypes.
    ...
    dim : int
        the domain dimension of the transformation (either 2 or 3)
    theta : array, shape(n)
        the buffer to write the identity parameters. The total number of
        parameters, n, is the sum of the parameters of the m transformations
    """
    cdef:
        int i, p, q, t
        int n = ttypes.shape[0]
        double[:] tmp = np.zeros_like(theta)
    with nogil:
        p = 0
        for i in range(n):
            t = ttypes[i]
            q = _number_of_parameters(t, dim)
            _get_identity_parameters(t, dim, tmp)
            theta[p:(p + q)] = tmp[:q]
            p += q
...
def get_composite_matrix(int[:] ttypes, int dim, double[:] theta, double[:,:] T):
    ...
    components of the composite transform.

    Parameters
    ----------
    ttypes : array, shape(m,)
        the types of the components of the composite transform
    dim : int
        the domain dimension of the transformation (either 2 or 3)
    theta : array, shape(n,)
        the parameters of the composite transform (the concatenation of
        the parameters of all its components)
    T : array, shape (dim + 1, dim + 1)
        the buffer in which to write the transform matrix
    """
    cdef:
...
def get_composite_jacobian(int[:] ttypes, int dim, double[:] theta, double[:] x,
                           double[:,:] J):
    r""" Gets the Jacobian of the composite transform
    ...
    J : array, shape (dim + 1, dim + 1)
        the buffer in which to write the Jacobian
    """
    cdef:
        double[:] y = np.empty(dim + 1)
        double[:,:] A = np.empty(shape = (dim+1, dim+1), dtype=np.float64)
        double[:] ttmp = np.empty_like(theta)
        double[:,:] Jtmp = np.empty_like(J)
        int i, m, p, q
    m = ttypes.shape[0]
    y[:dim] = x[:dim]
    p = 0
